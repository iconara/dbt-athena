...
class AthenaCursor:
    ...
    def execute(
        ...
        self._with_iceberg_retries(self._await_completion)
        return self
    ...
    def _fetch(self, limit: Optional[int] = None) -> Iterable[Row]:
        ...
        while not (next_token is None and headers_skipped):
            ...
            if not headers_skipped:
                self._column_info = results_response["ResultSet"]["ResultSetMetadata"]["ColumnInfo"]
...
class AthenaConnection:
    ...
    def connect(self) -> Self:
        ...
        self._client = self.session.client("athena", region_name=self.credentials.region_name, config=config)
        return self
