...
class TestAthenaCursor:
    ...
    def test_rowcount_fetches_the_update_count(self, cursor, athena_client):
        ...
        athena_client.get_query_results.assert_called_once_with(QueryExecutionId="1234-abcd", MaxResults=1)
    ...
    def test_execute_ignores_throttling_errors_from_get_query_execution(self, cursor, athena_client, credentials):
        client_error = botocore.exceptions.ClientError({"Error": {"Code": "ThrottlingException"}}, "GetQueryExecution")
        state_sequence = [STATE_EVENT_QUEUED]
    ...
    def test_fetchmany_loads_only_as_many_rows_as_needed(self, cursor, athena_client):
        ...
        cursor.execute("SELECT * FROM table")
        cursor.fetchmany(10)
        athena_client.get_query_results.assert_called_once_with(QueryExecutionId="1234-abcd", MaxResults=11)

    def test_fetchmany_loads_full_pages_when_the_limit_is_larger_than_a_page(self, cursor, athena_client):
        data = [[f"2024-01-{(n + 1):02d}", f"{n}"] for n in range(10)]
        page = self._create_page(athena_client, [("dt", "date"), ("n", "int")], data)
        athena_client.get_query_results = mock.Mock(return_value=page)
        cursor.execute("SELECT * FROM table")
