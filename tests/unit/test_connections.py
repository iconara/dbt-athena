...
class TestAthenaConnection:
    ...
    def athena_client(self):
        client = mock.Mock()
        client.start_query_execution = mock.Mock(return_value={"QueryExecutionId": "query-execution-id"})
    ...
    def test_connect_creates_athena_client(self, credentials, session_factory, config_factory):
        ...
        session.client.assert_called_once_with("athena", region_name=AWS_REGION, config=config_factory())

    def test_connect_returns_self(self, credentials, session_factory, config_factory):
        connection = AthenaConnection(credentials, boto_session_factory=session_factory)
        connection.connect(boto_config_factory=config_factory)
        session_factory.assert_called_once_with(connection)
        assert connection == connection.connect()
...
STATE_EVENT_CANCELLED = {
    "QueryExecution": {"Status": {"State": "CANCELLED", "StateChangeReason": "Query was cancelled!"}}
}
...
class TestAthenaCursor:
    ...
    def cursor(self, athena_client, credentials, poll_delay, formatter):
        return AthenaCursor(
            athena_client, credentials, poll_delay=poll_delay, formatter=formatter, retry_interval_multiplier=0
        )
    ...
    def test_rowcount_does_not_fetch_when_the_update_count_has_already_been_fetched(self, cursor, athena_client):
        athena_client.get_query_results = mock.Mock(
            return_value={
                "ResultSet": {"Rows": [], "ResultSetMetadata": {"ColumnInfo": []}},
                "UpdateCount": 1234,
    ...
    def test_execute_retries_iceberg_commit_errors(self, cursor, athena_client, credentials):
        ...
        state_sequence += [STATE_EVENT_RUNNING, STATE_EVENT_ICEBERG_COMMIT_ERROR] * credentials.num_iceberg_retries
        state_sequence += [STATE_EVENT_SUCCEEDED]
        athena_client.get_query_execution = mock.Mock(side_effect=state_sequence)
        cursor.execute("INSERT INTO time SELECT NOW()")
        assert athena_client.start_query_execution.call_count == credentials.num_iceberg_retries + 1

    def test_execute_fails_after_too_many_iceberg_commit_errors(self, cursor, athena_client, credentials):
        state_sequence = [STATE_EVENT_QUEUED]
    ...
    def test_execute_retries_on_api_request_errors_from_start_query_execution(
        ...
        client_error = botocore.exceptions.ClientError({"Error": {"Code": exception_name}}, "StartQueryExecution")
    ...
    def test_execute_ignores_api_request_errors_from_get_query_execution(
        ...
        client_error = botocore.exceptions.ClientError({"Error": {"Code": exception_name}}, "GetQueryExecution")
        state_sequence = [STATE_EVENT_QUEUED]
    ...
    def test_description_does_not_load_column_metadata_when_already_loaded_by_fetch(self, cursor, athena_client):
        cursor.execute("SELECT * FROM table")
    ...
    def _create_page(self, athena_client, column_info, rows, include_header=True, next_token=None):
        formatted_rows = [{"Data": [({"VarCharValue": v} if v is not None else {}) for v in row]} for row in rows]
        if include_header:
    ...
    def test_fetchmany_returns_fewer_rows_than_requested_when_there_are_no_more_rows(self, cursor, athena_client):
    ...
    def test_fetchmany_returns_as_many_rows_as_requested_when_there_are_more_rows(self, cursor, athena_client):
        data = [[f"2024-01-{(n + 1):02d}", f"{n}"] for n in range(10)]
        page = self._create_page(athena_client, [("dt", "date"), ("n", "int")], data)
        athena_client.get_query_results = mock.Mock(return_value=page)
        cursor.execute("SELECT * FROM table")
    ...
    def test_fetchmany_loads_more_pages_as_needed(self, cursor, athena_client):
        ...
        page1 = self._create_page(athena_client, [("n", "int")], data[0:999], include_header=True, next_token="p2")
        page2 = self._create_page(athena_client, [("n", "int")], data[999:1999], include_header=False, next_token="p3")
        page3 = self._create_page(athena_client, [("n", "int")], data[1999:2999], include_header=False, next_token="p4")
        page4 = self._create_page(athena_client, [("n", "int")], data[2999:], include_header=False, next_token=None)
    ...
    def test_fetchmany_loads_more_pages_lazily(self, cursor, athena_client):
        data = [[f"{n}"] for n in range(3999)]
        page1 = self._create_page(athena_client, [("n", "int")], data[0:999], include_header=True, next_token="p2")
        page2 = self._create_page(athena_client, [("n", "int")], data[999:1999], include_header=False, next_token="p3")
        page3 = self._create_page(athena_client, [("n", "int")], data[1999:2999], include_header=False, next_token="p4")
    ...
    def test_fetchone_fetchmany_fetchall(self, cursor, athena_client):
        ...
        page1 = self._create_page(athena_client, [("n", "int")], data[0:999], include_header=True, next_token="p2")
        page2 = self._create_page(athena_client, [("n", "int")], data[999:], include_header=False, next_token=None)
    ...
    def test_fetchone_fetchmany_fetchall_cannot_be_called_on_failed_cursor(self, cursor, athena_client):
        athena_client.get_query_execution = mock.Mock(side_effect=[STATE_EVENT_QUEUED, STATE_EVENT_GENERIC_ERROR])
    ...
    def test_fetchone_fetchmany_fetchall_cannot_be_called_on_cancelled_cursor(self, cursor, athena_client):
        athena_client.get_query_execution = mock.Mock(side_effect=[STATE_EVENT_QUEUED, STATE_EVENT_CANCELLED])
        with pytest.raises(AthenaError):
            cursor.execute("SELECT NOW()")
        with pytest.raises(AthenaError) as e:
            cursor.fetchone()
    ...
    def test_fetch_converts_timestamps(self, cursor, athena_client):
        ...
        rows = list(cursor.fetchall())
        assert rows == [
            [datetime.datetime(2024, 1, 1, 10, 11, 12, 13000)],
            [datetime.datetime(2024, 2, 29, 23, 59, 59, 999000)],
            [datetime.datetime(2024, 7, 6, 0, 0, 0, 0)],
            [datetime.datetime(2024, 12, 31, 0, 0, 0, 1000)],
        ]
    ...
    def test_fetch_converts_null(self, cursor, athena_client):
        types = ["varchar", "char", "int", "bigint", "float", "double", "boolean", "date", "timestamp", "time"]
